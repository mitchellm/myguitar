<?php
/**
 * This is the code that validates a checkout and processes it through the customers, orders and orderline tables
 */
...
if ($important == "checkout.php") {
    ...
    if ($session->isLoggedIn() && $totalItems > 0) {
        ...
        if ($session->hasBalance($masterTotal)) {
            $cartTotal = $store->getCartTotal();
            $tax = $cartTotal * .07;
            $ship = $cartTotal * .15;
            $session->reduceBalance($masterTotal);
            ...
            Utility::redirect("index.php?notice=Not enough balance to checkout the items in cart. Deposit more money!");
        }
    } else {
            Utility::redirect("index.php?notice=You must be logged in to place orders!");
    }
} else {
    $location = "index.php";
